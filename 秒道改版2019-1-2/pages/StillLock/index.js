...
Page({
  ...
  onLoad (options) {
    ...
    var that = this;
    that.setData({
      current: parseInt(options.uptm) ? parseInt(options.uptm) : 0,
      //current: 0
    })
  ...
  startTimer () {
    ...
    const t = setInterval(() => {
      ...
      this.setData({
        current: current,
        minutes: addPreceedingZero(current / 1000 / 60 / 60),
        seconds: addPreceedingZero(current / 1000 / 60 % 60),
        millseconds: addPreceedingZero(current / 1000 % 60),
      });
    }, 130);
    this.setData({
      start: start,
  ...
  alsolock() {
    ...
    if (!myreg.test(that.data.userphone)) {
      ...
      if (that.data.onoff) {
        ...
        //检测当前设备是否已经被还锁
        my.httpRequest({
          url: 'https://miaodaokeji.com/miaodao.php/Mdminiprogram/checkdevice',
          data: { dv_id: app.globalData.dv_id, userphone:that.data.userphone},
          method: 'POST',
          headers: {
            ...
            if(res.data.sta == 1){
              ...
              my.openBluetoothAdapter({  //初始化适配器
                success (res) {
                  ...
                  var opent = setTimeout(function () {  //还锁等待
  ...
  gxorde() {     //代还锁
    ...
    //更新订单
    my.httpRequest({
      url: 'https://miaodaokeji.com/miaodao.php/Mdminiprogramt/ralsolock',
      data: { dv_id: app.globalData.dv_id, openid: app.globalData.openId, userphone: that.data.userphone},
      method: 'POST',
      headers: {
        'content-type': 'application/x-www-form-urlencoded'
      },
      success: (res) => {
        ...
        if (res.data.status == 1) {
          ...
          clearTimeout(that.data.openlock);
          my.redirectTo({
            url: '../stillsuc/index?money=' + res.data.money + '&tmlen=' + res.data.tmlen + '&tuinum=' + res.data.tuinum + '&paytype=' + res.data.paytype +'&uptm='+res.data.uptm,
          })
